...
export function applyGrade({ state, grade, config, redCount, at, deviceId }: GradeInput): CardState {
  ...
  if (grade === 'good') {
    ...
    level = (config.passIsMinusOne || redCount < 10
      ? Math.max(state.level - 1, 0)
      : config.relearnToLevel) as CardLevel;
