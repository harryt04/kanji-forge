...
export interface ReplayOptions {
  config: SrsConfig
  redCount?: (deckId: string, states: ReadonlyMap<string, CardState>) => number
}
...
export function replay(
  ...
  for (const review of ordered) {
    ...
    const previous =
      states.get(key) ??
      emptyCardState(review.deckId, review.stickyId, review.deviceId)
    const state =
      review.source === 'manual'
        ? {
            ...previous,
            level: review.levelAfter,
            dueAt: nextDue(
              review.levelAfter,
              config,
              review.at,
              seededRandom(review.id),
            ),
            lastReviewedAt: review.at,
            manualOverride: true,
            updatedAt: review.at,
            updatedBy: review.deviceId,
          }
        : applyGrade({
            state: previous,
            grade: review.grade,
            config,
            redCount:
              redCount?.(review.deckId, states) ??
              countRed(review.deckId, states),
            at: review.at,
...
function countRed(
  ...
  for (const state of states.values())
    if (state.deckId === deckId && state.level === 0) count += 1
  return count
}
