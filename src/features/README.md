...
| `browse` | `/browse` | `browse-screen.tsx`, `browse-filter.ts`, `browse-sort.ts`, `browse-bulk.ts`, `browse-virtual.ts` | Tile-wall-first Browse with Search, Sort, Filter, View, and Select Menubar menus; View owns layout, tile content, tile zoom, and save-as-defaults, while Select owns selection mode and bulk flag/unflag/level operations. A slim sticky row reports the selected count. Lists above 500 cards use virtualized rendering. |
...
| `analytics` | n/a (root shell concern) | `posthog-init.tsx` | Production-only PostHog initialization when a key is configured; otherwise it is a no-op so unconfigured deployments still render normally. Autocaptured text and element attributes are masked, session recording is disabled, and App Router pageviews are captured manually. The component is mounted as a sibling in `src/app/layout.tsx`, not as a provider wrapper. |
