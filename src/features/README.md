...
| `browse` | `/browse` | `browse-screen.tsx`, `browse-filter.ts`, `browse-sort.ts`, `browse-bulk.ts`, `browse-virtual.ts` | Tile-wall-first Browse with Search, Sort, Filter, and View Menubar menus; View owns layout, tile content, tile zoom, and save-as-defaults, while selection and bulk flag/level operations remain available. Lists above 500 cards use virtualized rendering. |
