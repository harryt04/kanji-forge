...
export default function TileWall({ tiles }: TileWallProps): ReactNode {
  ...
  const [highZoomTiles, setHighZoomTiles] = useState<
    Array<{
      tileIdx: number
      x: number
      y: number
    ...
    (e: React.PointerEvent<HTMLCanvasElement>) => {
      ...
      if (pointersRef.current.size === 1) {
        ...
        if (rendererRef.current) {
          panTo(
            rendererRef.current,
            panStateRef.current.x,
            panStateRef.current.y,
          )
          scheduleRender()
        }
    ...
    (e: React.PointerEvent<HTMLCanvasElement>) => {
      if (
        ...
        if (canvas) {
          ...
          if (tileIdx >= 0) {
            console.log(
              `Tile hit at index ${tileIdx}: ${rendererRef.current.tiles[tileIdx]?.char || '?'}`,
            )
  ...
  return (
    <div className="relative flex-1">
      ...
      <div
        ...
        style={{
          display: highZoomTiles.length > 0 ? 'block' : 'none',
          overflow: 'hidden',
          position: 'relative',
        }}
      >
        {highZoomTiles.map(({ tileIdx, x, y, tile, tileSize }) => (
          <div
            ...
            onClick={() => {
              console.log(
                `Tile hit at index ${tileIdx}: ${tile.char || '?'} (Level: ${tile.level || 0})`,
              )
            }}
          >
            {tile.char}
