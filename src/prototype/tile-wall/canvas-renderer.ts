  ...
  if (useDirty) {
    ...
    if (panDeltaX !== 0) {
      ...
      const endCol = Math.min(
        gridWidth,
        Math.ceil((stripX + stripWidth - offsetX) / tileSize) + 1,
      )
      const startRow = Math.max(0, Math.floor(-offsetY / tileSize))
      const endRow = Math.min(
        gridHeight,
        Math.ceil((logicalH - offsetY) / tileSize) + 1,
      )
    ...
    if (panDeltaY !== 0) {
      ...
      const endCol = Math.min(
        gridWidth,
        Math.ceil((logicalW - offsetX) / tileSize) + 1,
      )
      const startRow = Math.max(0, Math.floor((stripY - offsetY) / tileSize))
      const endRow = Math.min(
        gridHeight,
        Math.ceil((stripY + stripHeight - offsetY) / tileSize) + 1,
      )
 ...
 */
export function setHitTestCallback(
  state: RendererState,
...
export function getVisibleTilesForDOM(
  ...
  const endCol = Math.min(
    gridWidth,
    Math.ceil((canvasWidth - offsetX) / tileSize) + 1,
  )
  const startRow = Math.max(0, Math.floor(-offsetY / tileSize))
  const endRow = Math.min(
    gridHeight,
    Math.ceil((canvasHeight - offsetY) / tileSize) + 1,
  )
