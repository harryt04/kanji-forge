...
export interface LocalUserDatabase {
  ...
  transaction(statements: readonly { readonly sql: string; readonly parameters?: SqlParameters }[]): Promise<void>;
  close(): void;
}
...
async function restoreFromOpfs(name: string): Promise<Uint8Array | undefined> {
  ...
  try {
    ...
    const file = await directory.getFileHandle(`${name}.sqlite`, { create: true });
    const contents = await file.getFile();
    return contents.size === 0 ? undefined : new Uint8Array(await contents.arrayBuffer());
  } catch {
...
export function openLocalUserDatabase(userId: string): LocalUserDatabase {
  if (!userId.trim()) throw new Error('An authenticated user id is required to open a local database.');
  ...
  const ready = (async (): Promise<void> => {
    ...
    migrateUserDatabase({
      ...
      hasMigration(version) {
        const statement = database?.prepare('SELECT 1 FROM schema_migrations WHERE version = ?', [version]);
        if (!statement) return false;
        const exists = statement.step();
        statement.free();
        return exists;
      },
      transaction(action) { database?.run('BEGIN'); try { action(); database?.run('COMMIT'); } catch (error) { database?.run('ROLLBACK'); throw error; } },
    }, Date.now());
    if (closed) {
      database.close();
      database = undefined;
  ...
  async function getDatabase(): Promise<SqlJsDatabase> {
    await ready;
    if (closed || !database) throw new Error('The local user database is closed.');
    return database;
  }
  ...
  return {
    ...
    async transaction(statements) {
      writeChain = writeChain.then(async () => {
        ...
        active.run('BEGIN');
        try {
          for (const statement of statements) active.run(statement.sql, statement.parameters ?? []);
          active.run('COMMIT');
        } catch (error) {
          active.run('ROLLBACK');
          throw error;
