...
| # | Feature | Status | Implemented | Notes |
|---|---|---|---|---|
| 9.1 | Paste Japanese text → readings, furigana, English | ⚠️ Partial | 2026-08-08 | Authenticated `/analyze` provides offline dictionary segmentation over written forms, kana readings, and common verb/adjective inflections—including polite-negative, past-negative, conditional, volitional, imperative, prohibitive, and progressive auxiliary forms (including polite negative)—with surface readings, plus grouped visible unknown-text fallback; a full morphological tokenizer pack remains the accuracy upgrade for grammatical segmentation |
