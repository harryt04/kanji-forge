...
| — | Deck sharing by file or URL | ➕ ✅ | 2026-08-08 | Settings can share any locally owned built-in, Saved, or custom deck as a content-only URL or JSON file; the authenticated `/analyze` route and Settings JSON importer preview dictionary-backed kanji and add them to Saved without sharing private SRS progress |
...
| # | Feature | Status | Implemented | Notes |
|---|---|---|---|---|
| 9.1 | Paste Japanese text → readings, furigana, English | ⚠️ Partial | 2026-08-08 | Authenticated `/analyze` provides offline dictionary segmentation over written forms, kana readings, common verb/adjective inflections, and a bounded grammar lexicon for particles and copula/auxiliary phrases—including potential, passive, causative, polite-negative, past-negative, conditional, volitional, imperative, prohibitive, and progressive auxiliary forms (including polite negative)—with surface readings, plus grouped visible unknown-text fallback. Furigana sentence alignments now use a shared validated parser with malformed-data fallback; a full morphological tokenizer pack remains the accuracy upgrade for grammatical segmentation |
