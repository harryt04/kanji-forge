...
| — | Deck sharing by file or URL | ➕ 🔶 | 2026-08-08 | Settings copies a content-only URL or downloads a content-only JSON share file for the built-in deck; the authenticated `/analyze` route and Settings JSON importer preview dictionary matches and add them to Saved without sharing private SRS progress |
...
| 8.3 | ~700,000 entries including names and places | 🔷 | 2026-08-08 | `npm run build:names` reproducibly builds the optional JMnedict SQLite pack; Dictionary searches and links available names when `names-v1.sqlite` is installed, while the default install remains lightweight |
...
| # | Feature | Status | Implemented | Notes |
|---|---|---|---|---|
| 9.1 | Paste Japanese text → readings, furigana, English | ⚠️ Partial | 2026-08-08 | Authenticated `/analyze` provides offline dictionary segmentation over written forms, kana readings, and common verb/adjective inflections—including polite-negative, past-negative, conditional, volitional, and progressive auxiliary forms (including polite negative)—with surface readings, plus grouped visible unknown-text fallback; a full morphological tokenizer pack remains the accuracy upgrade for grammatical segmentation |
...
| — | Optional self-hostable sync server | ➕ 🔷 | 2026-08-08 | `deploy/docker-compose.yml` runs Postgres, API, and pinned Electric; `/api/electric/shape` authenticates sessions, allow-lists sync tables, and injects a parameterized `user_id` filter before proxying the streaming shape response. |
| — | Authenticated read synchronization | ➕ ✅ | 2026-08-08 | The API exposes a user-scoped sync snapshot and the authenticated runtime polls it, merges review ids by union, and applies metadata last-write-wins. It is transport-compatible with the planned Electric shape; direct Electric streaming remains a deployment follow-up. |
| — | Authenticated mutation write API | ➕ ✅ | 2026-08-08 | Self-hosted API accepts authenticated batches for reviews and deck/settings/membership metadata, stamps the session user id, applies reviews idempotently, and resolves metadata last-write-wins; direct Electric read streaming remains a deployment follow-up |
