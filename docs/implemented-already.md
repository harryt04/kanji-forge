...
| Content-pack access | `src/data/packs/index.ts` | Read-only SQLite-WASM access to the `packs-dev` fixture packs: loads deck definitions from `decks.json`, looks up kanji by literal or classical radical, searches the dictionary, and parses `type:key` content refs (e.g. `kanji:日`). Explicitly documented as a dev fixture, pending the full pack manager (`T2.2`). |
