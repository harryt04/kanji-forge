...
| Detail view | `src/features/detail/detail-screen.tsx`, `src/features/detail/stroke-animation.tsx` | The `/detail` route renders offline kanji metadata, ordered KanjiVG stroke playback with accessible play/pause/restart/step controls, ranked examples and similar kanji, Saved-deck membership, and previous/next navigation through the loaded deck with horizontal touch swiping. |
