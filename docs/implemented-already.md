...
**Status date:** 2026-08-07 — based on a direct read of the source tree, not committed history
...
| Browse list | `src/features/browse/browse-screen.tsx`, `src/features/browse/browse-filter.ts`, `src/app/browse/page.tsx` | The `/browse` route loads the authenticated user's built-in deck from the local database and renders an accessible list with each card's kanji, readings, meanings, level/belt label, and flag state. The list has offline text search across kanji, kana readings, and English meanings plus level/color, flagged, stroke-count range, and JLPT filters. Tile view is persisted per user and can show kanji, reading, or first meaning content at a persisted 75%, 100%, or 150% zoom ratio; users can explicitly save those three Browse choices as defaults for future decks, with deck-specific choices taking precedence. |
| Detail view | `src/features/detail/detail-screen.tsx`, `src/features/detail/stroke-animation.tsx` | The `/detail` route renders offline kanji metadata, ordered KanjiVG stroke playback with accessible play/pause/restart/step controls, labeled device-synthesized Japanese audio when available, ranked examples and similar kanji, Saved-deck membership, and previous/next navigation through the loaded deck with horizontal touch swiping. |
...
| Content-pack access | `src/data/packs/index.ts` | Read-only SQLite-WASM access to the `packs-dev` fixture packs: loads deck definitions from `decks.json`, looks up kanji by literal or classical radical, searches the dictionary with exact/prefix/substring and wildcard matching, and parses `type:key` content refs (e.g. `kanji:日`). Explicitly documented as a dev fixture, pending the full pack manager (`T2.2`). |
...
| Theme, study style, and automatic backup settings | `src/features/settings/{theme,theme-controller,settings-screen,auto-backup}.ts*`, `src/features/study/study-style.ts` | Authenticated users can choose light, dark, device, or StickyStudy-compatible 21:00–06:00 night mode, choose the study question face, independently select answer fields, enable two-tap word → readings → details reveals, and choose a desktop backup folder. The folder handle is stored per account; when permission remains granted, a complete JSON backup is written at most once per day as the app opens or returns to the foreground. Choices are stored locally and applied offline. |
...
| PWA registration and app badge | `src/pwa/index.ts`, `src/pwa/app-badge.ts`, `src/pwa/sw.ts`, `next.config.js` | Registers the build-generated Serwist worker in the browser; the worker precaches build assets and caches visited navigations for offline reloads. Updated workers wait rather than interrupting an active study session. On supported installed browsers, a persisted per-user setting controls an app-icon badge showing due/new cards, total cards, or no badge. | T5.0 |
