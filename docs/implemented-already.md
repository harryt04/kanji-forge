...
| Detail view | `src/features/detail/detail-screen.tsx` | The `/detail` route renders offline kanji metadata, ranked examples and similar kanji, Saved-deck membership, and previous/next navigation through the loaded deck with horizontal touch swiping. |
| Authenticated navigation | `src/features/navigation/app-navigation.tsx`, `src/auth/auth-gate.tsx` | Protected screens share primary links for Home, Study, Browse, History, and Dictionary. Browse displays a locally loaded sticky-count badge from the installed starter deck and keeps the link usable if pack loading fails. |
...
| Theme and study style settings | `src/features/settings/{theme,theme-controller,settings-screen}.ts*`, `src/features/study/study-style.ts` | Authenticated users can choose light, dark, device, or StickyStudy-compatible 21:00–06:00 night mode, choose the study question face, and independently select answer fields. Choices are stored in the local user database and applied offline. |
