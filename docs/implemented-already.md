...
| Offline text analyzer | `src/features/share/share-screen.tsx`, `src/data/packs/index.ts`, `src/core/text/analyzer.ts` | Authenticated `/analyze` accepts pasted Japanese text and displays dictionary-backed longest-match tokens, bounded particle/copula grammar tokens, ruby readings, English meanings, links to offline word/kanji details, and explicit unknown-character fallback; a full tokenizer pack remains a future accuracy improvement. Covered by `share-screen.test.tsx`, `analyzer.test.ts`, and `packs/index.test.ts`. |
...
fallback now syncs the five server projections), local-only mutation types do not yet have a server projection, writing
