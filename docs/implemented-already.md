...
| Import/enrichment | `src/core/import/{parse,enrich,apkg}.ts` | Pure CSV/TSV/line parsers handle quoted cells, multiline values, BOMs, comments, and compact kanji lists. Pure enrichment helpers deduplicate content identities, merge duplicate tags, and classify matched, already-in-target, and unresolved entries; Anki `.apkg` parsing preserves Japanese word runs, individual kanji, and note tags for offline dictionary preview and sticky-annotation import. Covered by `src/core/import/import.test.ts` and `deck-import.test.ts`. | Anki scheduling and card templates remain intentionally unmigrated |
