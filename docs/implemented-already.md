...
| Web Share Target | `src/features/share/share-screen.tsx`, `src/app/analyze/page.tsx`, `public/manifest.json` | The installed PWA can receive shared text at `/analyze`, show an authenticated offline kanji preview, and add matched cards to Saved without changing SRS progress. Covered by `src/features/share/share-screen.test.tsx`. |
| Offline text analyzer | `src/features/share/share-screen.tsx`, `src/data/packs/index.ts` | Authenticated `/analyze` accepts pasted Japanese text and displays dictionary-backed longest-match tokens with ruby readings, English meanings, and explicit unknown-character fallback; a tokenizer pack remains a future accuracy improvement. Covered by `share-screen.test.tsx` and `packs/index.test.ts`. |
