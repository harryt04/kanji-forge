...
| Authenticated mutation ingest | `apps/api/src/mutations.ts`, `apps/api/src/index.ts` | The self-hosted API validates batches of review and metadata mutations, derives `user_id` from the authenticated better-auth session, inserts reviews idempotently, and applies deck/settings/membership metadata with timestamp-based last-write-wins semantics. Covered by `mutations.test.ts`; Electric read sync remains an explicit follow-up. |
...
What still does not work end-to-end: multi-device sync (Electric read sync is still a stub, and
local-only mutation types do not yet have a server projection), writing
