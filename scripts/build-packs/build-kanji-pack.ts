...
function parseKanjidic(gzPath: string): {
  entries: KanjiEntry[]
  skippedBadQc: number
} {
  ...
  while ((charMatch = charBlockRegex.exec(xml)) !== null) {
    ...
    const cpMatch = /<cp_value cp_type="ucs">([0-9a-fA-F]+)<\/cp_value>/.exec(
      block,
    )
    if (!cpMatch) continue
  ...
  try {
    ...
    const insertTx = db.transaction((list: KanjiEntry[]) => {
      for (const e of list) {
        ...
        const alt =
          e.strokeCount.length > 1
            ? JSON.stringify(e.strokeCount.slice(1))
            : null
        insert.run({
          literal: e.literal,
    ...
    const cols: Array<{ name: string }> = db
      .prepare(`PRAGMA table_info(kanji)`)
      .all() as any
    for (const c of cols) {
      if (/skip|misclass|heisig|query_code|dic_ref|qc_type/i.test(c.name)) {
        throw new Error(
          `ASSERTION FAILED: disallowed column name in schema: ${c.name}`,
        )
      }
    }
    ...
    if (
      ...
      throw new Error(
        'ASSERTION FAILED: qc_type= or skip marker found in DB bytes',
      )
    }

    const countRow = db
      .prepare('SELECT COUNT(*) as count FROM kanji')
      .get() as {
      count: number
    }
...
async function main() {
  ...
  console.log(
    `✓ Input SHA256 verified against lock: ${actualSha.slice(0, 16)}...`,
  )
