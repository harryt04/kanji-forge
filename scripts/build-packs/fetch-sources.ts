  ...
  return new Promise((resolve, reject) => {
    ...
    const request = protocol.get(urlStr, options, (response) => {
      ...
      if (response.statusCode !== 200) {
        reject(
          new Error(`Failed to fetch ${urlStr}: HTTP ${response.statusCode}`),
        )
        return
      }
  ...
  return new Promise((resolve, reject) => {
    ...
    const request = protocol.get(urlStr, (response) => {
      if (response.statusCode === 302 || response.statusCode === 301) {
        ...
        downloadFile(locUrl.toString(), filePath, label)
          .then(resolve)
          .catch(reject)
        return
      }
...
async function computeLicenseHash(
...
const LICENSE_URLS: Record<string, string> = {
  kanjidic2: 'https://www.edrdg.org/edrdg/licence.html',
  jmdict: 'https://www.edrdg.org/edrdg/licence.html',
  jmnedict: 'https://www.edrdg.org/edrdg/licence.html',
  kradfile: 'https://www.edrdg.org/edrdg/licence.html',
  radkfile: 'https://www.edrdg.org/edrdg/licence.html',
  kanjivg:
    'https://raw.githubusercontent.com/KanjiVG/kanjivg/r20260714/COPYING',
  jmdictfurigana:
    'https://raw.githubusercontent.com/Doublevil/JmdictFurigana/master/LICENSE',
  tatoeba: 'https://tatoeba.org/eng/terms_of_use',
  'jlpt-kanji-data':
    'https://raw.githubusercontent.com/davidluzgouveia/kanji-data/master/LICENSE',
  'jlpt-vocab-yomitan':
    'https://raw.githubusercontent.com/stephenmk/yomitan-jlpt-vocab/master/LICENSE',
  'noto-serif-jp':
...
function sourceParts(
  lock: any,
): Array<{ id: string; entry: any; parent?: any }> {
  ...
  for (const [id, source] of Object.entries<any>(lock.sources ?? {})) {
    ...
    for (const [componentId, component] of Object.entries<any>(
      source.components ?? {},
    ))
      parts.push({
        id: `${id}.${componentId}`,
...
export function assertLicenseChangesAllowed(
  previous: any,
  candidate: any,
  approved: boolean,
): void {
  const changes = Object.entries<any>(candidate.sources ?? {}).flatMap(
    ([id, entry]) => {
      const old = previous.sources?.[id]
      return old?.licenseHash &&
        entry.licenseHash &&
        old.licenseHash !== entry.licenseHash
        ? [`${id}: ${old.licenseHash} -> ${entry.licenseHash}`]
        : []
    },
  )
  if (changes.length && !approved)
    throw new Error(
      `LICENSE CHANGE DETECTED; refusing to replace sources.lock.json. Review and rerun with --approve-license-change:\n${changes.join('\n')}`,
    )
}

function assertContract(lock: any, requireFiles = false): void {
  for (const { id, entry } of sourceParts(lock)) {
    if (!entry.url || !entry.sha256 || !/^[a-f0-9]{64}$/.test(entry.sha256))
      throw new Error(
        `Invalid locked source ${id}: url and sha256 are required`,
      )
    const file = canonicalFile(entry, id.replace('.', '-'))
    if (path.basename(file) !== file)
      throw new Error(
        `Invalid locked source ${id}: file must be a cache filename`,
      )
    if (
      requireFiles &&
      entry.cacheRequired !== false &&
      !fs.existsSync(path.join(CACHE_DIR, file))
    )
      throw new Error(`Missing locked cache input ${id}: ${file}`)
  }
  for (const [id, source] of Object.entries<any>(lock.sources ?? {}))
    if (!/^[a-f0-9]{64}$/.test(source.licenseHash ?? ''))
      throw new Error(`Invalid locked license hash for ${id}`)
}
...
function assertPlaceholderRejected(placeholder: string, id: string): void {
  if (
    /^[0-9a-f]{64}$/.test(placeholder) &&
    placeholder !== 'cc-by-sa-4.0-edrdg'
  ) {
...
async function getLatestKanjiVGRelease(): Promise<{
  tag: string
  assetUrl: string
}> {
  ...
  try {
    const releases = await fetchJson(
      'https://api.github.com/repos/KanjiVG/kanjivg/releases?per_page=1',
    )
    ...
    console.warn(
      '  (Warning: Could not fetch latest KanjiVG release, using fallback)',
    )
  }
  return {
    tag: 'r20260714',
    assetUrl:
      'https://github.com/KanjiVG/kanjivg/releases/download/r20260714/kanjivg-20260714-all.zip',
  }
}

async function getLatestYomitanRelease(): Promise<{
  tag: string
  assetUrl: string
}> {
  ...
  try {
    const releases = await fetchJson(
      'https://api.github.com/repos/stephenmk/yomitan-jlpt-vocab/releases?per_page=1',
    )
    if (Array.isArray(releases) && releases.length > 0) {
      const release = releases[0]
    ...
    console.warn(
      '  (Warning: Could not fetch latest yomitan JLPT release, using fallback)',
    )
  }
  return {
    tag: '2025.08.01.0',
    assetUrl:
      'https://github.com/stephenmk/yomitan-jlpt-vocab/releases/download/2025.08.01.0/jlpt-vocab-2025.08.01.0.zip',
  }
}
...
async function getLatestKanjiDataCommit(): Promise<string> {
  ...
  try {
    const data = await fetchJson(
      'https://api.github.com/repos/davidluzgouveia/kanji-data/commits?per_page=1',
    )
    ...
    console.warn(
      '  (Warning: Could not fetch latest kanji-data commit, using fallback)',
    )
  }
...
async function buildSourcesList(): Promise<SourceEntry[]> {
  ...
  return [
    ...
    },
    {
      ...
      sha256: '', // Will be computed
      provenance:
        'JmdictFurigana alignment DATA (Doublevil), derivative of JMdict (EDRDG) under CC BY-SA 4.0; repository code is separately MIT-licensed.',
    },
    {
    ...
    },
    {
      ...
      sha256: '', // Will be computed
      provenance:
        "Derived from Jonathan Waller's JLPT Resources (tanos.co.uk), CC BY",
    },
    {
...
async function fetchAllSources(
  ...
  for (const source of sources) {
    ...
    assertPlaceholderRejected(
      source.licenseHash || 'cc-by-sa-4.0-edrdg',
      source.id,
    )
...
async function main() {
  ...
  if (isPreflight) {
    console.log(
      '✓ Refresh preflight passed: checked-out lock has canonical builder cache inputs.',
    )
    return
  }
  ...
  for (const { id, entry, parent } of sourceParts(candidate)) {
    ...
    if (
      entry.derivedFile &&
      !fs.existsSync(path.join(CACHE_DIR, entry.derivedFile))
    )
      execFileSync('tar', ['-xzf', target, '-C', CACHE_DIR])
  }
  ...
  console.log('\n=== Verification ===')
  console.log(
    `All sources have licenseHash: ${Object.values(lock.sources).every((s) => s.licenseHash)}`,
  )
}
