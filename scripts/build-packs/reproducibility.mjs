...
function build(workspace) {
  ...
  for (const builder of builders) {
    execFileSync(join(root, 'node_modules/.bin/tsx'), [`scripts/build-packs/${builder}`], {
      cwd: workspace,
      stdio: 'inherit',
    });
  }
...
try {
  const one = join(work, 'one'); const two = join(work, 'two');
  for (const dir of [one, two]) mkdirSync(dir);
  const oneOut = build(one); const twoOut = build(two);
  const left = files(oneOut).map(file => relative(oneOut, file)).sort(); const right = files(twoOut).map(file => relative(twoOut, file)).sort();
  if (JSON.stringify(left) !== JSON.stringify(right)) throw new Error('rebuild artifact sets differ');
  for (const file of left) if (!readFileSync(join(oneOut, file)).equals(readFileSync(join(twoOut, file)))) throw new Error(`rebuild differs: ${file}`);
  console.log(`✓ Reproducible full rebuild passed (${left.length} pack files, manifests, and Brotli artifacts byte-compared in isolated output directories)`);
