...
async function loadWWWJDICIndices(): Promise<Map<number, TatoebaIndicesEntry>> {
  ...
  for await (const line of rl) {
    ...
    for (const tok of tokens) {
      ...
      if (surface) {
        words.push({ headword: headword || surface, reading, entSeq, senseId, surface });
      }
    }
...
function computeReadabilityScore(ja: string, en: string): number {
  ...
  for (const char of ja) {
    ...
    if ((code >= 0x4e00 && code <= 0x9fff) || (code >= 0xf900 && code <= 0xfaff)) {
      kanjiCount++;
    }
  }
...
function applyFurigana(
    ...
    if (start >= 0) {
      ...
      if (furiEntry && furiEntry.furigana.length > 0) {
        ...
        if (wordSurface === word.headword) {
          ...
          segments.push({ text: wordSurface, furigana: joinedRt || word.reading });
        } else {
        ...
        const furiText = (word.reading && word.headword === wordSurface) ? word.reading : '';
  ...
  if (reconstructed !== ja) {
    console.warn(`[applyFurigana] reconstruct !== ja (len ${reconstructed.length} vs ${ja.length}); some tokens may have drifted`);
  }
  return segments;
}
...
async function buildSentencesPack() {
  console.log('Starting Tatoeba sentences ETL (memory-efficient streaming + per-sense cap)...\n');
  ...
  for await (const line of rl) {
    ...
    if (linkCount % 500000 === 0) {
      console.log(`  Processed ${linkCount} links, candidates so far: ${candidateSentences.length}...`);
    }
    ...
    for (const word of indicesEntry.words) {
      ...
      senseToCandidates.get(senseKey)!.push({ sentenceId: jpnId, score: readabilityScore });
    }
  }

  console.log(`  Considered ${considered} sense-linked sentence pairs after filters.`);
  ...
  for (let i = 0; i < wordLinks.length; i += BATCH) {
    ...
    for (const ln of batch) {
      insertLink.bind([ln.sentenceId, ln.jmdictSenseId, ln.wordStartChar, ln.wordEndChar]);
      insertLink.step();
      insertLink.reset();
    }
  }
