...
export function assertSourceIntegrity(
  ...
  assert(
    lock.sources && typeof lock.sources === 'object',
    'sources.lock.json has no sources',
  )
  for (const [id, source] of Object.entries(lock.sources)) {
    ...
    const matching = candidates.some(
      (filePath) => sha256(readFileSync(filePath)) === source.licenseHash,
    )
    if (candidates.length > 0 || requireCached)
      assert(matching, `${id} license hash changed`)
  }
  for (const [id, source] of sourceEntries(lock.sources)) {
    ...
    const file =
      source.file ??
      (!id.includes('.')
        ? `${source.id}-${source.pinned}${extname(new URL(source.url).pathname) || '.bin'}`
        : undefined)
    if (file && existsSync(join(cacheDir, file))) {
      assert(
        sha256(readFileSync(join(cacheDir, file))) === source.sha256,
        `${id} source hash changed`,
      )
...
export function manifestFiles(packDir) {
  return walk(packDir).filter(
    (file) =>
      basename(file) === 'manifest.json' || file.endsWith('.manifest.json'),
  )
}
...
function derivedCount(manifestFile, manifest) {
  ...
  if (
    ['kanji', 'words-core', 'words-full', 'names', 'sentences'].includes(
      manifest.id,
    )
  ) {
    const table = {
    ...
    const database = new Database(join(dir, `${stem}.sqlite`), {
      readonly: true,
    })
    try {
  ...
  if (manifest.id === 'similar')
    return Object.keys(readJson(join(dir, 'similar.json'))).length
  if (manifest.id === 'decks')
    return Array.isArray(manifest.decks) ? manifest.decks.length : undefined
  if (Array.isArray(manifest.chunks))
    return manifest.chunks.reduce(
      (total, chunk) =>
        total + Object.keys(readJson(join(dir, chunk.filename))).length,
      0,
    )
  return undefined
}

export function assertPackManifests(
  packDir,
  baselines,
  budgets,
  fixture = false,
  ...
  for (const file of manifests) {
    ...
    assert(
      ...
      `${manifest.id} manifest lacks license`,
    )
    assert(
      typeof manifest.attribution === 'string' && manifest.attribution.trim(),
      `${manifest.id} manifest lacks attribution`,
    )
    const count = derivedCount(file, manifest)
    const claim = claimedCount(manifest)
    const baseline = (fixture ? baselines.fixtureCounts : baselines.counts)[
      manifest.id
    ]
    if (baseline !== undefined) {
      assert(
        typeof count === 'number',
        `${manifest.id} has no derivable artifact count`,
      )
      assert(
        claim === count,
        `${manifest.id} manifest count ${claim} does not match derived artifact count ${count}`,
      )
      const drift = (Math.abs(count - baseline) / baseline) * 100
      assert(
        drift <= baselines.maxDriftPercent,
        `${manifest.id} count drift ${drift.toFixed(2)}% exceeds ${baselines.maxDriftPercent}%`,
      )
    }
    const budget = (fixture ? budgets.fixtureMaxBytes : budgets.maxBytes)[
      manifest.id
    ]
    if (budget !== undefined) {
      const size = Array.isArray(manifest.brotli)
        ? manifest.brotli.reduce(
            (total, artifact) => total + artifact.sizeBytes,
            0,
          )
        : (manifest.sizeBytes ?? manifest.catalogSizeBytes)
      assert(
        typeof size === 'number' && size <= budget,
        `${manifest.id} exceeds declared pack budget`,
      )
    }
  }
  return manifests
...
export function assertNoSkipFields(packDir) {
  ...
  const forbidden =
    /(?:qc_type["']?\s*[=:]\s*["']?(?:skip|misclass)|skip_misclass|"(?:skip|misclass)"\s*:)/i
  for (const file of walk(packDir).filter(
    (file) =>
    ...
    assert(
      !forbidden.test(readFileSync(file).toString('utf8')),
      `surviving SKIP/misclassification field in ${file}`,
    )
  }
}
...
function artifactsForManifest(manifestFile, manifest) {
  const dir = join(manifestFile, '..')
  if (Array.isArray(manifest.chunks))
    return manifest.chunks.map((chunk) => join(dir, chunk.filename))
  if (Array.isArray(manifest.decks))
    return manifest.decks.map((deck) => join(dir, deck.file))
  if (manifest.id === 'similar') return [join(dir, 'similar.json')]
  const stem = basename(manifestFile).replace('.manifest.json', '')
  return ['.sqlite', '.json']
    .map((extension) => join(dir, `${stem}${extension}`))
    .filter(existsSync)
}

export function assertAndWriteDeterministicBrotli(
  packDir,
  { write = false } = {},
) {
  const result = []
  for (const manifestFile of manifestFiles(packDir)) {
    ...
    for (const artifact of artifactsForManifest(manifestFile, manifest)) {
      assert(
        existsSync(artifact),
        `${manifest.id} artifact missing: ${artifact}`,
      )
      ...
      assert(
        first.equals(second),
        `${manifest.id} Brotli output is not deterministic`,
      )
      const record = {
        file: `${basename(artifact)}.br`,
        sha256: sha256(first),
    ...
    assert(
      compressed.length > 0,
      `${manifest.id} has no publishable artifact to Brotli-compress`,
    )
    if (write) {
...
export function runAssertions({
  ...
  const baselines = readJson(
    join(root, 'scripts/build-packs/pack-baselines.json'),
  )
  const budgets = readJson(join(root, 'scripts/build-packs/pack-budgets.json'))
  assertNoSkipFields(packDir)
  const compressed = assertAndWriteDeterministicBrotli(packDir, {
    write: writeBrotli,
  })
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  try {
    ...
    const base = fixture
      ? join(root, 'scripts/build-packs/fixtures')
      : join(root, 'scripts/build-packs/.cache')
    const records = runAssertions({
      packDir: fixture ? join(root, 'packs-dev') : join(root, 'packs'),
      lockPath: fixture
        ? join(base, 'sources.lock.json')
        : join(root, 'scripts/build-packs/sources.lock.json'),
      cacheDir: base,
      fixture,
    ...
    console.log(
      `✓ Phase 0 ${mode} pipeline passed (${records.length} deterministic Brotli artifacts checked)`,
    )
  } catch (error) {
