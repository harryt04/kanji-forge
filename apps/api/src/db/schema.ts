...
}, (table) => [primaryKey({ columns: [table.id, table.userId] }), index('decks_user_id_idx').on(table.userId)]);
...
export const deckMembership = pgTable('deck_membership', {
  userId: text('user_id').notNull().references(() => user.id, { onDelete: 'cascade' }),
  deckId: text('deck_id').notNull(),
  contentRef: text('content_ref').notNull(),
  ...
  updatedAt: timestamp('updated_at', { withTimezone: true }).notNull(),
}, (table) => [primaryKey({ columns: [table.userId, table.deckId, table.contentRef] }), index('deck_membership_user_id_idx').on(table.userId)]);
