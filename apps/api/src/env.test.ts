...
describe('readEnv', () => {
  it('requires the backend secrets and applies safe network defaults', () => {
    expect(readEnv(validEnv)).toMatchObject({ API_HOST: '0.0.0.0', API_PORT: 3001 });
  });

  it('rejects an absent or short better-auth secret', () => {
    expect(() => readEnv({ ...validEnv, BETTER_AUTH_SECRET: '' })).toThrow('BETTER_AUTH_SECRET');
    expect(() => readEnv({ ...validEnv, BETTER_AUTH_SECRET: 'too-short' })).toThrow('at least 32');
  });
});
